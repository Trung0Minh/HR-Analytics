...
def group_experience(data, col_name='experience'):
    """
    Groups the 'experience' column into specified categories by creating a new array with an updated dtype.

    Args:
        data (np.ndarray): The input structured array.
        col_name (str): The name of the experience column.

    Returns:
        np.ndarray: A new array with the 'experience' column updated and dtype adjusted.
    """
    if col_name not in data.dtype.names:
        return data

    # Logic to create `grouped_exp` array from the original `experience` column
    exp_numeric = np.zeros(len(data), dtype=int)
    for i, exp_str in enumerate(data[col_name]):
        if exp_str == '<1': exp_numeric[i] = 0
        elif exp_str == '>20': exp_numeric[i] = 21
        elif exp_str == '': exp_numeric[i] = -1
        else:
            try: exp_numeric[i] = int(exp_str)
            except (ValueError, TypeError): exp_numeric[i] = -1

    conditions = [
        exp_numeric == 0,
        (exp_numeric >= 1) & (exp_numeric <= 5),
        (exp_numeric >= 6) & (exp_numeric <= 10),
        (exp_numeric >= 11) & (exp_numeric <= 15),
        (exp_numeric >= 16) & (exp_numeric <= 20),
        exp_numeric > 20
    ]
    choices = ['<1', '1-5', '6-10', '11-15', '16-20', '>20']
    grouped_exp = np.select(conditions, choices, default='')

    # Create a new dtype list, modifying the dtype for the experience column
    new_dtype = []
    max_len = max(len(c) for c in choices) if choices else 5
    for name, dtype_str in data.dtype.descr:
        if name == col_name:
            new_dtype.append((name, f'<U{max_len}'))
        else:
    ...
    # Copy data, replacing the old experience column with the new grouped one
    for name in data.dtype.names:
        if name == col_name:
            new_data[name] = grouped_exp
        else:
