...
def fill_missing_categorical(data, columns, fill_value='Missing'):
    """
    Thay thế các chuỗi rỗng trong các cột phân loại được chỉ định bằng một giá trị điền.
    Hàm này tạo ra một mảng mới với các kiểu dữ liệu (dtypes) được cập nhật để chứa giá trị điền,
    ngăn chặn việc cắt bớt dữ liệu.

    Args:
        data (np.ndarray): Mảng cấu trúc đầu vào.
        columns (list): Một danh sách các tên cột cần xử lý.
        fill_value (str): Chuỗi để thay thế các giá trị trống.

    Returns:
        np.ndarray: Một mảng mới với các giá trị bị thiếu đã được điền và các kiểu dữ liệu đã được điều chỉnh.
    """
    new_dtype_descr = list(data.dtype.descr)
    
    # Tạo bản đồ tra cứu để truy cập nhanh hơn
    dtype_map = {name: i for i, (name, _) in enumerate(new_dtype_descr)}

    # Cập nhật dtypes cho các cột được chỉ định để đảm bảo chúng có thể chứa fill_value
    for col in columns:
        # Kiểm tra xem cột có tồn tại không
        if col not in dtype_map:
            continue

        # Lấy dtype và độ dài hiện tại
        idx = dtype_map[col]
        dtype_str = new_dtype_descr[idx][1]

        # Chỉ xử lý các kiểu chuỗi
        if 'U' in dtype_str:
            try:
                # Trích xuất độ dài chính xác từ chuỗi như '<U15'
                current_len = int(dtype_str[2:]) 
                if len(fill_value) > current_len:
                    new_dtype_descr[idx] = (col, f'<U{len(fill_value)}')
            except (ValueError, IndexError):
                # Dự phòng cho các kiểu dữ liệu không ở định dạng mong đợi
                pass

    # Tạo một mảng mới với dtype mới
    new_data = np.empty(data.shape, dtype=np.dtype(new_dtype_descr))

    # Sao chép dữ liệu và điền các giá trị bị thiếu
    for name in data.dtype.names:
        if name in columns:
            original_col = data[name]
            # Sử dụng np.where để điền các giá trị bị thiếu một cách hiệu quả
...
def encode_ordinal(data, column_name, category_mapping):
    """
    Mã hóa một cột phân loại bằng cách sử dụng ánh xạ thứ tự được chỉ định.
    Bất kỳ giá trị nào không có trong ánh xạ sẽ được đặt thành -1.

    Args:
        data (np.ndarray): Mảng cấu trúc đầu vào.
        column_name (str): Tên của cột cần mã hóa.
        category_mapping (dict): Một từ điển ánh xạ các danh mục thành các số nguyên.

    Returns:
        np.ndarray: Một mảng mới với cột đã được mã hóa và kiểu dữ liệu đã được cập nhật.
    """
    if column_name not in data.dtype.names:
        return data

    # Tạo một mảng mới cho cột được mã hóa, mặc định là -1
